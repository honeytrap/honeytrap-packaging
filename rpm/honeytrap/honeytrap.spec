...
%global commit          48161f4162e535e766cc9c5955ca384990366e62
%global commitdate      20181103
...
BuildRequires: golang(github.com/dgraph-io/badger/options)
...
Requires:      golang(github.com/dgraph-io/badger/options)
...
Provides:      golang(%{import_path}/pushers/marija) = %{version}-%{release}
...
Provides:      golang(%{import_path}/services/filesystem) = %{version}-%{release}
...
Provides:      golang(%{import_path}/services/smtp) = %{version}-%{release}
...
# Temporary fix to avoid tests producing garbage in buildroot, fix should be merged upstream
sed -i '22s/SetDataDir("")/SetDataDir("\/tmp")/' services/smtp/smtp_test.go

# Install systemd unit file
cat << EOF > honeytrap.service
[Unit]
...
install -p config.toml.sample %{buildroot}%{_sysconfdir}/honeytrap.toml
install -d -p %{buildroot}%{_unitdir}
install -p honeytrap.service %{buildroot}%{_unitdir}
...
%gotest %{import_path}/pushers/marija
...
%gotest %{import_path}/services/filesystem
%gotest %{import_path}/services/ftp
%gotest %{import_path}/services/ipp
%gotest %{import_path}/services/smtp
...
%post
%systemd_post honeytrap.service

%preun
%systemd_preun honeytrap.service

%postun
%systemd_postun_with_restart honeytrap.service
...
%{_bindir}/honeytrap
#%%{_bindir}/gen-ldflags
%config(noreplace) %{_sysconfdir}/honeytrap.toml
%{_unitdir}/honeytrap.service
...
%changelog
* Mon Mar 12 2018 Athos Ribeiro <athoscr@fedoraproject.org> - 0-0.2.20181103git48161f4
- Update revision
